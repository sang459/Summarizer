...
def summary(raw_text):
    # raw_text를 split하기
    text_splitter = TokenTextSplitter(chunk_size=3000, chunk_overlap=30)
    ...
    SUMMARY WITH IMPORTANT DETAILS (using markdown headings and bulletpoints):"""
    ...
    refine_template = (
        ...
        "{text}\n"
        "------------\n"
        "Produce a detailed summary of the given part of the article, using headings and bullet points.\n"
        "Omit the overlapping part of the summary."
    )
    ...
    chain = load_summarize_chain(OpenAI(temperature=0.1, openai_api_key=OPENAI_API_KEY), chain_type="refine", 
